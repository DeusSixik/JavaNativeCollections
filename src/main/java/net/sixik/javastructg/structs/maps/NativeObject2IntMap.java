...
public final class NativeObject2IntMap<K> extends NativeMap {
    ...
    private NativeIntArray values;
    private long statesAddress;
    private long hashesAddress;
    private long keysAddress;
    private long valuesAddress;

    public NativeObject2IntMap(int initialCapacity, NativeTypeMemory<K> keyMemory) {
        this(initialCapacity, keyMemory, null);
    }

    public NativeObject2IntMap(int initialCapacity, NativeTypeMemory<K> keyMemory, Supplier<K> keyBufferFactory) {
        ...
        this.values = new NativeIntArray(capacity);
        refreshAddresses();
    }
    ...
    public boolean putPrehashed(K key, long hash, int value) {
        ...
        while (true) {
            ...
            if (state == EMPTY) {
                ...
                UNSAFE.putInt(valueAddress(target), value);
                size++;
                return true;
            }
            ...
            if (state == DELETED) {
                ...
                UNSAFE.putInt(valueAddress(index), value);
                return false;
            }
    ...
    public int getPrehashed(K key, long hash, int defaultValue) {
        int mixedHash = spreadHash(hash);
        int index = indexForHash(mixedHash);
        ...
        while (true) {
            ...
            if (state == USED
                ...
                return UNSAFE.getInt(valueAddress(index));
            }
    ...
    protected void rehash(int newCapacity) {
        ...
        NativeIntArray oldValues = values;
        ...
        values = new NativeIntArray(capacity);
        refreshAddresses();
        size = 0;
        usedSlots = 0;
        ...
        for (int i = 0; i < oldCapacity; i++) {
            if (UNSAFE.getByte(oldStatesAddress + i) == USED) {
                reinsert(
                        ...
                        UNSAFE.getInt(oldValues.ptr() + (((long) i) << 2))
                );
            }
        }
    ...
    private void reinsert(int mixedHash, long oldKeyAddress, int value) {
        int index = indexForHash(mixedHash);
    ...
    private long valueAddress(int index) {
        return valuesAddress + (((long) index) << 2);
    }
