...
public final class NativeObject2ObjectMap<K, V> extends NativeMap {
    ...
    public NativeObject2ObjectMap(
            int initialCapacity,
            NativeTypeMemory<K> keyMemory,
            NativeTypeMemory<V> valueMemory
    ) {
        this(initialCapacity, keyMemory, null, valueMemory);
    }
    ...
    public NativeObject2ObjectMap(
            ...
            NativeTypeMemory<K> keyMemory,
            NativeTypeMemory<V> valueMemory,
            Supplier<K> keyBufferFactory
    ) {
        this(initialCapacity, keyMemory, keyBufferFactory, valueMemory);
    }
    ...
    public boolean putPrehashed(K key, long hash, V value) {
        ...
        while (true) {
            ...
            if (state == EMPTY) {
                ...
                valueMemory.writeToMemory(UNSAFE, valueAddress(target), value);
                size++;
                return true;
            }
    ...
    public boolean get(K key, V outValue) {
        return getPrehashed(key, keyMemory.hash(key), outValue);
    }
        ...
        for (int i = 0; i < oldCapacity; i++) {
            if (UNSAFE.getByte(oldStatesAddress + i) == USED) {
                reinsert(
                        ...
                        oldValuesAddress + (((long) i) * valueSize)
                );
            }
        }
    ...
    private void reinsert(int mixedHash, long oldKeyAddress, long oldValueAddress) {
        int index = indexForHash(mixedHash);
