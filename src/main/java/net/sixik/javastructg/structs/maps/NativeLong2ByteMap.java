...
public final class NativeLong2ByteMap extends NativeMap {
    ...
    public NativeLong2ByteMap(int initialCapacity) {
    ...
    public boolean put(long key, byte value) {
        ...
        while (true) {
            ...
            if (state == EMPTY) {
                ...
                UNSAFE.putByte(valuesAddress + target, value);
                size++;
                return true;
            }
            ...
            if (state == DELETED) {
                ...
                UNSAFE.putByte(valuesAddress + index, value);
                return false;
            }
    ...
    public byte get(long key, byte defaultValue) {
        ...
        while (true) {
            ...
            if (state == USED && UNSAFE.getLong(longAddress(keysAddress, index)) == key) {
                return UNSAFE.getByte(valuesAddress + index);
            }
    ...
    private void reinsert(long key, byte value) {
        int index = indexForKey(key);
