...
public final class NativeInt2IntMap extends NativeMap {
    ...
    public NativeInt2IntMap(int initialCapacity) {
    ...
    public boolean put(int key, int value) {
        ...
        while (true) {
            ...
            if (state == EMPTY) {
                ...
                UNSAFE.putInt(intAddress(valuesAddress, target), value);
                size++;
                return true;
            }
            ...
            if (state == DELETED) {
                ...
                UNSAFE.putInt(intAddress(valuesAddress, index), value);
                return false;
            }
    ...
    public int get(int key, int defaultValue) {
        ...
        while (true) {
            ...
            if (state == USED && UNSAFE.getInt(intAddress(keysAddress, index)) == key) {
                return UNSAFE.getInt(intAddress(valuesAddress, index));
            }
    ...
    protected void rehash(int newCapacity) {
        ...
        for (int i = 0; i < oldCapacity; i++) {
            if (UNSAFE.getByte(oldStatesAddress + i) == USED) {
                reinsert(UNSAFE.getInt(intAddress(oldKeysAddress, i)), UNSAFE.getInt(intAddress(oldValuesAddress, i)));
            }
        }
