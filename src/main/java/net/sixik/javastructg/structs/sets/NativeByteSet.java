...
public final class NativeByteSet extends NativeSet {

    private NativeByteArray states;
    private NativeByteArray keys;

    public NativeByteSet(int initialCapacity) {
        super(initialCapacity);
        this.states = new NativeByteArray(capacity);
        this.keys = new NativeByteArray(capacity);
    }
    ...
    protected void rehash(int newCapacity) {
        ...
        NativeByteArray oldKeys = keys;
        int oldCapacity = capacity;

        capacity = newCapacity;
        mask = capacity - 1;
        states = new NativeByteArray(capacity);
        keys = new NativeByteArray(capacity);
        size = 0;
        usedSlots = 0;
