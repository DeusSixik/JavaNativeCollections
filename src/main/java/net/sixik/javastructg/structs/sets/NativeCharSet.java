...
public final class NativeCharSet extends NativeSet {

    private NativeByteArray states;
    private NativeCharArray keys;

    public NativeCharSet(int initialCapacity) {
        super(initialCapacity);
        this.states = new NativeByteArray(capacity);
        this.keys = new NativeCharArray(capacity);
    }
    ...
    protected void rehash(int newCapacity) {
        ...
        NativeCharArray oldKeys = keys;
        int oldCapacity = capacity;

        capacity = newCapacity;
        mask = capacity - 1;
        states = new NativeByteArray(capacity);
        keys = new NativeCharArray(capacity);
        size = 0;
        usedSlots = 0;
