...
public final class NativeShortSet extends NativeSet {

    private NativeByteArray states;
    private NativeShortArray keys;

    public NativeShortSet(int initialCapacity) {
        super(initialCapacity);
        this.states = new NativeByteArray(capacity);
        this.keys = new NativeShortArray(capacity);
    }
    ...
    protected void rehash(int newCapacity) {
        ...
        NativeShortArray oldKeys = keys;
        int oldCapacity = capacity;

        capacity = newCapacity;
        mask = capacity - 1;
        states = new NativeByteArray(capacity);
        keys = new NativeShortArray(capacity);
        size = 0;
        usedSlots = 0;
