...
public final class NativeBooleanSet extends NativeSet {

    private NativeByteArray states;
    private NativeBooleanArray keys;

    public NativeBooleanSet(int initialCapacity) {
        super(initialCapacity);
        this.states = new NativeByteArray(capacity);
        this.keys = new NativeBooleanArray(capacity);
    }

    public boolean add(boolean value) {
        ensureCapacityForInsert();

        int index = NativeUtils.mix(value ? 1 : 0) & mask;
        int firstDeleted = -1;
    ...
    protected void rehash(int newCapacity) {
        ...
        NativeBooleanArray oldKeys = keys;
        int oldCapacity = capacity;

        capacity = newCapacity;
        mask = capacity - 1;
        states = new NativeByteArray(capacity);
        keys = new NativeBooleanArray(capacity);
        size = 0;
        usedSlots = 0;
